...
class MODPOTrainer(Trainer):
    ...
    def get_preference_batch(self, batch_data, group_size=4):
        with torch.no_grad():
            ...
            # NDCG: Higher Rank = Better
            rank_ndcg = all_ndcg.argsort(dim=1).argsort(dim=1).float()
            norm_ndcg = rank_ndcg / (group_size) 
            
            # Pop: Higher Rank = Lower Pop (Better)
            rank_pop = (-all_pop).argsort(dim=1).argsort(dim=1).float()
            norm_pop = rank_pop / (group_size)
            
            # 4. Alpha Scoring
            final_scores = (self.w_alpha * norm_ndcg) + ((1 - self.w_alpha) * norm_pop)
            
            # disqualify garbage immediately from being a "Winner" 
            # (Winner must be at least somewhat relevant)
            ...
            # --- 6. The "Hybrid" Safety Mask (The Fix) ---
            
            # Gather metrics for Winner vs Loser
            chosen_score = final_scores[batch_indices, best_indices]
            rejected_score = final_scores[batch_indices, worst_indices]
            
            chosen_pop_val = all_pop[batch_indices, best_indices]
            rejected_pop_val = all_pop[batch_indices, worst_indices]
            
            rejected_ndcg_val = all_ndcg[batch_indices, worst_indices]
            
            # Condition A: General Score Improvement
            is_better_score = (chosen_score > rejected_score)
            
            # Condition B: Serendipity Constraint
            # "Winner must have Lower Pop"
            is_more_serendipitous = (chosen_pop_val < rejected_pop_val)
            
            # Condition C: Safety Exception
            # If the Loser is "Garbage" (NDCG=0), we allow the Winner to be popular.
            # We must verify the Winner is actually valid (Score > -1)
            is_loser_garbage = (rejected_ndcg_val == 0)
            
            # FINAL MASK:
            # Train if: (Better Score) AND ( (Loser is Garbage) OR (Winner is More Serendipitous) )
            valid_mask = is_better_score & (is_loser_garbage | is_more_serendipitous) & (chosen_score > -0.5)
